...
GRASS_RADIUS = 1000.0 # decrease for evolved agents, increase for random agents
MAX_ENERGY = 100.0
...
# save data
DATA_PATH = "./data/random/training2/"
# load agent data
...
def save_run_metadata(data_path, seed, generations, agent_condition, status, best_generation=None, best_loss=None):
    metadata = {
        ...
        "random_brains_per_generation": True,
        "seed": int(seed),
...
def generate_random_brains(key):
    key, sheep_rand_key, wolf_rand_key = jax.random.split(key, 3)
    sheep_params_tiled = jax.random.uniform(
        sheep_rand_key, (MAX_SHEEP, NUM_ES_PARAMS), minval=INIT_LOW, maxval=INIT_HIGH
    )
    wolf_params_tiled = jax.random.uniform(
        wolf_rand_key, (MAX_WOLF, NUM_ES_PARAMS), minval=INIT_LOW, maxval=INIT_HIGH
    )
    return key, sheep_params_tiled, wolf_params_tiled

def main():
    args = parse_args()
    data_path = normalize_data_path(args.data_path)
    os.makedirs(data_path, exist_ok=True)
    save_run_metadata(data_path, args.seed, args.generations, "random", "running")
    ...
    print("Generating fresh random brains at each generation...")
    ...
    best_loss = jnp.inf
    best_generation = -1
    best_sheep_params_tiled = None
    best_wolf_params_tiled = None

    for gen in range(args.generations):
        key, sheep_params_tiled, wolf_params_tiled = generate_random_brains(key)
        ...
        if gen_best_loss < best_loss:
            ...
            best_generation = gen
            best_sheep_params_tiled = sheep_params_tiled
            best_wolf_params_tiled = wolf_params_tiled
            ...
            jnp.save(data_path + 'phase3_checkpoint_sheep_brain_params.npy', best_sheep_params_tiled)
            jnp.save(data_path + 'phase3_checkpoint_wolf_brain_params.npy', best_wolf_params_tiled)
    ...
    if best_sheep_params_tiled is not None and best_wolf_params_tiled is not None:
        jnp.save(data_path + 'phase3_sheep_brain_params.npy', best_sheep_params_tiled)
        jnp.save(data_path + 'phase3_wolf_brain_params.npy', best_wolf_params_tiled)
    save_run_metadata(data_path, args.seed, args.generations, "random", "complete", best_generation, best_loss)
