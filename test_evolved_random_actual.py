...
def generate_random_actual_env_params(key):
    """
    Generate one random 12-parameter ABM environment directly in actual parameter space.
    ...
    actual_params = jax.random.uniform(
        env_key,
        shape=(12,),
        minval=sim.MIN_BOUNDS,
        maxval=sim.MAX_BOUNDS,
    )

    return key, actual_params
...
def load_evolved_brain_params():
    sheep_path = os.path.join(resolve_sim_path(sim.SHEEP_BRAIN_PATH), "sheep_param_list_mean.npy")
    wolf_path = os.path.join(resolve_sim_path(sim.WOLF_BRAIN_PATH), "wolf_param_list_mean.npy")

    if not os.path.exists(sheep_path):
        raise FileNotFoundError(f"Could not find evolved sheep brain params at {sheep_path}")
    if not os.path.exists(wolf_path):
        raise FileNotFoundError(f"Could not find evolved wolf brain params at {wolf_path}")

    print(f"Loading evolved sheep brain parameters from {sheep_path}")
    print(f"Loading evolved wolf brain parameters from {wolf_path}")
    sheep_params = jnp.array(np.load(sheep_path)[-1])
    wolf_params = jnp.array(np.load(wolf_path)[-1])
...
def main():
    ...
    parser.add_argument("--eval-steps", type=int, default=15000)
    ...
    print(f"Generating random actual ABM parameters directly inside bounds from seed {args.seed}...")
    key, constrained_params = generate_random_actual_env_params(key)

    if constrained_params.shape[0] != 12:
        raise ValueError(f"Expected 12 random actual ABM parameters, got shape {constrained_params.shape}")
    ...
    print("Random actual ABM parameters:")
    print(
    ...
    eval_sheep_params, eval_wolf_params = load_evolved_brain_params()
    ...
    jnp.save(data_path + "random_actual_abm_params.npy", constrained_params)
